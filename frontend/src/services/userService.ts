...
export const userService = {
  ...
  async listUsers(skip = 0, limit = 100): Promise<User[]> {
    const { data } = await api.get<User[]>('/api/v1/users', { params: { skip, limit } })
    return data
  },
  ...
  async updateUser(userId: string, userData: UserUpdate): Promise<User> {
    const { data } = await api.patch<User>(`/api/v1/users/${userId}`, userData)
    return data
  },
  ...
  async getRole(roleId: string): Promise<Role> {
    const { data } = await api.get<Role>(`/api/v1/roles/${roleId}`)
    return data
  },

  async createRole(roleData: RoleCreate): Promise<Role> {
    const { data } = await api.post<Role>('/api/v1/roles', roleData)
    return data
  },

  async updateRole(roleId: string, roleData: RoleUpdate): Promise<Role> {
    const { data } = await api.patch<Role>(`/api/v1/roles/${roleId}`, roleData)
    return data
  },
  ...
  async listGroups(): Promise<UserGroup[]> {
    const { data } = await api.get<UserGroup[]>('/api/v1/groups')
    return data
  },

  async getGroup(groupId: string): Promise<UserGroup> {
    const { data } = await api.get<UserGroup>(`/api/v1/groups/${groupId}`)
    return data
  },

  async createGroup(groupData: UserGroupCreate): Promise<UserGroup> {
    const { data } = await api.post<UserGroup>('/api/v1/groups', groupData)
    return data
  },

  async updateGroup(groupId: string, groupData: UserGroupUpdate): Promise<UserGroup> {
    const { data } = await api.patch<UserGroup>(`/api/v1/groups/${groupId}`, groupData)
    return data
  },
